...
{
    LOG_INFO("TcpConnection::dtor[%s] at fd=%dd state=%d\n", name_.c_str(), channel_->fd(), (int)state_);
}
...
// 发送数据,应用写的快,而内核发的慢, 需要把待发送数据写入缓冲区,而且设置了水位回调
void TcpConnection::sendInLoop(const void *message, size_t len)
{
    ...
    // 调用过该connection的shutdown,不能再发送数据
    ...
    if (!channel_->isWriting() && outputBuffer_.readableBytes() == 0)
    {
        ...
        if (nwrote >= 0)
        {
            ...
            {
                // 既然这里所有数据全部发送完成,就不用再给channel设置epollout事件了
    ...
    /*当前这一次write,没有把数据全部发送出去,剩余的数据需要保存到缓冲区当中
    ...
    if (!faultError && remaining > 0)
    {
        ...
        {
            channel_->enableWriting(); // channel若未注册写事件则注册写事件,这样poller才会通知channel epollout
        }
    }
}
// 关闭连接
...
void TcpConnection::handleClose()
{
    ...
    connectionCallback_(connPtr); // 检查连接状态等待回调
